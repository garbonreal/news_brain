...
class NjKjjSpider(scrapy.Spider):
    name = "njkjj"
    start_urls = ["http://www.njkj.gov.cn/kjzx/gzyw/index.shtml"]

    url = "http://www.njkj.gov.cn/kjzx/gzyw/index_{}.shtml"
    page_num = 1
    ...
    def parse(self, response):
        li_list = response.xpath("//div[@class='list-right-con']/ul/li")
        # 将new_list数据写入到文件中

        for li in li_list:
            news_title = li.xpath(".//a/text()").extract_first()
            news_link = "http://www.njkj.gov.cn/kjzx/gzyw" + li.xpath(".//a/@href").extract_first().replace("./", "/")
