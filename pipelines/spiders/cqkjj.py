...
class CqkjjGovSpider(scrapy.Spider):
    name = "cqkjj"
    start_urls = ["https://kjj.cq.gov.cn/zwxx_176/kjdt/index.html"]

    url = "https://kjj.cq.gov.cn/zwxx_176/kjdt/index_{}.html"
    page_num = 1
    ...
    def parse(self, response):
        li_list = response.xpath("//div[@class='page-list clearfix']/ul/li")
        # 将new_list数据写入到文件中

        for li in li_list:
            news_title = li.xpath(".//a/text()").extract_first()
            news_link = "https://kjj.cq.gov.cn/zwxx_176/kjdt" + li.xpath(".//a/@href").extract_first().replace("./", "/")
