...
class BeijingGovSpider(scrapy.Spider):
    name = "beijinggov"
    start_urls = ["http://jxj.beijing.gov.cn/jxdt/gzdt/index.html"]

    url = "http://jxj.beijing.gov.cn/jxdt/gzdt/index_{}.html"
    page_num = 1
    ...
    def parse(self, response):
        ...
        li_list = response.xpath("//div[@class='box']/ul/li")
        # 将new_list数据写入到文件中

        for li in li_list:
            news_title = li.xpath(".//a/text()").extract_first().strip()
            news_link = "http://jxj.beijing.gov.cn/jxdt/gzdt" + li.xpath(".//a/@href").extract_first().replace("./", "/")
            news_time = li.xpath(".//span[@class='date']/text()").extract_first().strip().replace("[", "").replace("]", "")
