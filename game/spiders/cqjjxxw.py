...
class CqjjxxwSpider(scrapy.Spider):
    name = "cqjjxxw"
    start_urls = ["https://jjxxw.cq.gov.cn/zwgk_213/zcwj/qtwj/index.html"]

    url = "https://jjxxw.cq.gov.cn/zwgk_213/zcwj/qtwj/index_{}.html"
    page_num = 1
    ...
    def parse(self, response):
        li_list = response.xpath("//div[@class='content centerWidth']/div[@class='right']/ul/li")
        # 将new_list数据写入到文件中

        for li in li_list:
            news_title = li.xpath(".//a/text()").extract_first()
            news_link = "https://jjxxw.cq.gov.cn/zwgk_213/zcwj/qtwj" + li.xpath(".//a/@href").extract_first().replace("./", "/")
